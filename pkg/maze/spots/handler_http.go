...
func MapHandlers(m *mux.Router, endpoints Endpoints) {
	...
	m.Handle("/spots", httptransport.NewServer(
	...
	m.Handle("/spots/{id}", httptransport.NewServer(
...
func DecodeUpdateRequest(_ context.Context, r *http.Request) (interface{}, error) {
	req := UpdateRequest{}
	err := json.NewDecoder(r.Body).Decode(&req.Spot)
	if err != nil {
		return nil, err
	}
	if err = req.Spot.Validate(); err != nil {
		return nil, err
	}
	req.Spot.ID, _ = primitive.ObjectIDFromHex(mux.Vars(r)["id"])
